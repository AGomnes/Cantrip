...
6. **Release**: the Godot addon is done and packaged — node, importer, export check, editor dock, debugger tabs showing a running game's causality tree and what each live entity is made of, and a demo. A packaged build has been run to prove content reaches it. Still to do: breakpoints and stepping (which need a core change), a docs site and a cookbook.
