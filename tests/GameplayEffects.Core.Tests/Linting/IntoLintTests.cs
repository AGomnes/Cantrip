...
namespace GameplayEffects.Tests.Linting
{
    ...
    /// that the linter does not know about makes every use of the name it binds warn GE302. That is
    ...
    public sealed class IntoLintTests
    {
        private static IReadOnlyList<Diagnostic> Lint(string dsl) =>
            Linter.Lint(ContentLibrary.FromText(dsl, "into-lint.ge"), null);
        ...
        public void A_name_written_on_a_verb_that_ignores_into_is_still_unknown()
        {
            ...
            // false negative.
            Assert.Contains(diagnostics, d => d.Code == "GE302");
        }
    }
}
