...
namespace GameplayEffects.GodotAdapter.Tests.Shared
{
    ...
    /// </summary>
    public sealed class GeDebugServiceTests
    {
        private const string File = "res://content/cards.ge";
...
";
        ...
        private static GeDebugService Started(out Entity slime)
        {
            ...
            return new GeDebugService(runtime);
        }

        [Fact]
        public void A_game_introduces_itself()
        {
            GeDebugService service = Started(out Entity _);

            GeHello hello = service.Hello();

            Assert.Equal(GeProtocol.Version, hello.Protocol);
        ...
        {
            GeDebugService service = Started(out Entity slime);
        ...
        public void Saving_a_file_changes_the_running_game()
        {
            ...
            GeReloadResult result = service.Reload(new[]
            {
        ...
        public void The_console_runs_statements_and_reports_what_fails()
        {
            ...
            GeExecuteResult ok = service.Execute("deal 4 to enemy");
            Assert.True(ok.Ok, ok.Message);
            Assert.Equal(26, slime.GetInt("hp"));

            GeExecuteResult nonsense = service.Execute("deal 4 to whatever");
