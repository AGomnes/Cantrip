...
namespace GameplayEffects.GodotAdapter.Tests.Shared
{
    ...
    public sealed class SharedLayerTests
    {
        ...
        public void No_shared_source_reaches_for_the_engine()
        {
            ...
            {
                // Prose may name the engine; code may not. The namespace GameplayEffects.GodotAdapter
        ...
        public void The_whole_shared_layer_is_compiled_here()
        {
            ...
            foreach (string file in files)
            {
                ...
                if (Type.GetType("GameplayEffects.GodotAdapter." + name + ", GameplayEffects.Godot.Tests") == null)
                    missing.Add(name);
            }
        ...
        private static string SharedDirectory()
        {
            ...
            while (directory != null && !File.Exists(Path.Combine(directory.FullName, "GameplayEffects.sln")))
            {
            ...
            string shared = Path.Combine(
                directory!.FullName, "godot", "GameplayEffects.Demo", "addons", "gameplay_effects", "shared");
