...
namespace GameplayEffects.GodotAdapter.Tests.Shared
{
    ...
    /// </summary>
    public sealed class GeProtocolTests
    {
        private static TraceLog Log(int entries, int? capacity = null)
        {
            ...
            {
                log.Record(i, "action", "step " + i, "Card#1", span: new SourceSpan("res://content/cards.ge", i, 3, 4));
            }
            return log;
        }
        ...
        {
            Assert.Equal("gameplay_effects:trace", GeProtocol.Message(GeProtocol.Trace));

            Assert.True(GeProtocol.TryName("gameplay_effects:trace", out string withPrefix));
            Assert.Equal("trace", withPrefix);

            // Godot strips the prefix in one direction and not the other, so both are accepted.
            Assert.True(GeProtocol.TryName("trace", out string bare));
        ...
        {
            Assert.False(GeProtocol.TryName("something_else:trace", out _));
            Assert.False(GeProtocol.TryName("gameplay_effects:", out _));
            Assert.False(GeProtocol.TryName("", out _));
            Assert.False(GeProtocol.TryName(null, out _));
        }
        ...
        public void An_entry_carries_the_line_that_produced_it()
        {
            ...
            log.Record(
                ...
                new SourceSpan("res://content/cards.ge", 12, 5, 6),
            ...
            Assert.Equal("res://content/cards.ge", entry.File);
