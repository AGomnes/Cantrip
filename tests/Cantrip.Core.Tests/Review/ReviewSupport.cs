...
namespace Cantrip.Tests.Review
{
    ...
    /// <summary>
    /// Runs the cantrip CLI in a child process. Defects that overflow the stack kill the whole
    ...
    internal static class ReviewCli
    {
        ...
        private static string FindCli()
        {
            ...
            {
                if (!File.Exists(Path.Combine(dir.FullName, "Cantrip.sln"))) continue;

                string bin = Path.Combine(dir.FullName, "src", "Cantrip.Cli", "bin");
                string? cli = Directory.Exists(bin)
                    ? Directory.GetFiles(bin, "cantrip.dll", SearchOption.AllDirectories).OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault()
                    : null;
                return cli ?? throw new InvalidOperationException("cantrip.dll not found; build the solution before running the review tests.");
            }
            throw new InvalidOperationException("Could not find Cantrip.sln above the test output folder.");
        }
    }
}
