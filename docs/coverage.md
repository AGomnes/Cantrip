...
- The summary table is bookkeeping over the sections below it, and nothing checks the two against each other. One figure went stale unnoticed: a row moved from workaround to works, the README was updated to match, and the total was not, so the table and the README disagreed for a commit. Every summary row and the total have since been counted against the section rows, and all nine games agree at 92 effects. The section rows are the authority — count them rather than adjusting the total by hand.
