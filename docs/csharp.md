...
The rest of a battle screen reads the same state:
...
int drawPile = runtime.State.ZoneOf(player, Zones.Draw).Count;       // likewise Discard, Exhaust, Relics
```
...
{
    public Queue<GameEvent> Events { get; } = new Queue<GameEvent>();

    // Presentation hangs off events. Record them here and show them later: see the next section.
    public override void OnEvent(GameEvent gameEvent) => Events.Enqueue(gameEvent);
...
Events arrive in the order they complete, so an event that wraps others comes after them: a card that hits twice and applies a status reports `damaged`, `damaged`, `status_applied`, then `card_played`. Listeners resolve after the action that triggered them (unless the [ruleset](language.md#rulesets) says `triggers: immediate`), so whatever they cause arrives after it too.
...
sealed class BattleScreen
{
    ...
    public void EndTurnPressed()
    {
        ...
        if (runtime.Pending != null) ShowChoice(runtime.Pending);   // see Player choices
    }
    ...
    public void Update(float seconds)
    {
        ...
        else if (host.Events.Count > 0) showing = Present(host.Events.Dequeue());
    }
    ...
    private float Present(GameEvent gameEvent)
    {
        switch (gameEvent.Name)
        {
            ...
            case "card_played":
                return ShowCardPlayed(gameEvent.Card!);
            default:
                return 0;
        }
    }
...
Not everything raises an event. A played card reaches the discard pile, and the hand is discarded at the end of the turn, without one, and `<stat>_changed` is raised only when some content listens for it. So when the queue is empty, redraw the hand, the piles and the bars from the live state.
