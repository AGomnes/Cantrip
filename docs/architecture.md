...
| `Content/BlockAddresses`: `BlockAddressBook.Add` and `ExecutedStatements.Schedules` | a scheduled block nested inside the node has no address, so a game holding it cannot be saved |
| `Content/BlockAddresses`: `BlockHash` | the hash saves record stops noticing edits inside the node, so a save can run a changed block. `BlockHashTests` lists the nodes it knows and fails until the new one is added |
...
A new part of a listener's `on` line, such as another modifier after `once per ...`, needs a line in `BlockHash` too. Saves record a hash of each listener with a used limit or a timer, and a part it leaves out lets two different listeners hash alike, so a restored save could give one listener's limit to the other. `BlockHashTests` lists the parts it knows and fails until the new one is added.
