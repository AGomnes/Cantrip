    ...
    public sealed class Lexer
    {
        ...
        private void ReadToken()
        {
            ...
            switch (c)
            {
                ...
                case '!':
                    ...
                    _diagnostics.Error("GE0002", "`!` must be part of `!=`.", SpanAt(start, 1));
                    return;

                default:
                    _diagnostics.Error("GE0003", $"Unexpected character `{c}`.", SpanAt(start, 1));
                    return;
            }
        }
        ...
        private void ReadString(char quote)
        {
            ...
            while (_position < _text.Length)
            {
                ...
                if (c == '\n' || c == '\r')
                {
                    _diagnostics.Error("GE0005", "String is missing its closing quote.", SpanAt(start, _position - start));
                    break;
                }
