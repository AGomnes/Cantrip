...
namespace GameplayEffects.GodotAdapter
{
    ...
    public static class GodotContentLoader
    {
        ...
        /// (GE01xx parsing, GE3xx-GE4xx linting) because it is an adapter problem, not a content
        /// problem: the text never reached the parser.
        /// </summary>
        public const string UnreadableCode = "GE0901";
        ...
        /// <summary>
        /// Every <c>.ge</c> file under a folder, ordinally sorted. A missing folder is a warning
        ...
        public static IReadOnlyList<string> Discover(string folder)
        {
            ...
            {
                GD.PushWarning($"Gameplay Effects: no content folder at {folder}.");
                return found;
            }
        ...
        public static bool TryRead(string resPath, out string text)
        {
            ...
            if (ResourceLoader.Exists(resPath))
            {
                ...
                if (loaded is GeContentFile imported)
                {
        ...
        private static void Walk(string folder, List<string> found, int depth)
        {
            if (depth > MaxDepth)
            {
                GD.PushWarning($"Gameplay Effects: stopped looking for content more than {MaxDepth} folders below {folder}.");
                return;
            }

            foreach (string file in DirAccess.GetFilesAt(folder))
            {
                // An exported project lists cards.ge.remap where the editor lists cards.ge, and the
