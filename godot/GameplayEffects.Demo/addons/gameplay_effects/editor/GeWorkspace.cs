...
namespace GameplayEffects.GodotAdapter
{
    ...
    /// </para>
    /// </remarks>
    public sealed class GeWorkspace
    {
        /// <summary>Where content lives, so a project can keep <c>.ge</c> files in one folder.</summary>
        public const string ContentFolderSetting = "gameplay_effects/content/folder";

        /// <summary>Verbs a game registers from C#, so GE301 does not fire on every one of them.</summary>
        public const string HostVerbsSetting = "gameplay_effects/lint/host_verbs";

        /// <summary>Events a game raises from C#, for GE304 and GE305.</summary>
        public const string HostEventsSetting = "gameplay_effects/lint/host_events";

        /// <summary>Names a game's host resolves, for GE302.</summary>
        public const string HostNamesSetting = "gameplay_effects/lint/host_names";

        /// <summary>Lint codes this user does not want to see, kept per user in the editor settings.</summary>
        public const string SuppressedSetting = "gameplay_effects/lint/suppressed_codes";

        /// <summary>
        /// Reported when the linter itself fails. It sits in the adapter's own GE09xx range beside
        /// <see cref="GodotContentLoader.UnreadableCode"/>, because it says something about the
        /// tools rather than about the content.
        /// </summary>
        public const string LintFailedCode = "GE0902";
        ...
        public static bool IsSuppressable(string? code) =>
            !string.IsNullOrEmpty(code) && (code!.StartsWith("GE3", StringComparison.Ordinal) || code.StartsWith("GE4", StringComparison.Ordinal));
        ...
        private IReadOnlyList<Diagnostic> Lint(ContentLibrary library)
        {
            ...
            {
                GD.PushError($"Gameplay Effects: the linter failed. {error.Message}");
                return new[]
                {
                    new Diagnostic(
        ...
        /// <summary>Splits a setting written as <c>GE306, GE310</c> or <c>GE306 GE310</c>.</summary>
