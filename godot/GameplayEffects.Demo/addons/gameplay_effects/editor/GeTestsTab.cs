...
namespace GameplayEffects.GodotAdapter
{
    ...
    /// <remarks>
    /// <para>
    /// The runner is the core's own, so a test that passes here passes in <c>gedsl test</c> and in
    ...
    /// </remarks>
    [Tool]
    public partial class GeTestsTab : VBoxContainer
    {
        ...
        public void Initialize(GeWorkspace workspace, Action<string, int, int> navigate)
        {
        ...
        public void RunAll()
        {
            ...
            catch (Exception error) when (!(error is OutOfMemoryException))
            {
                ...
                GD.PushError($"Gameplay Effects: the test runner failed. {error.Message}");
