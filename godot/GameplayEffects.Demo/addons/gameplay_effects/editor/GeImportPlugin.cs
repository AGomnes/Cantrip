...
{
    /// <summary>
    /// Imports <c>.ge</c> files into <see cref="GeContentFile"/> resources, so that content is
    ...
    /// </para>
    /// <para>
    /// <see cref="_GetResourceType"/> returns "Resource" rather than "GeContentFile" on purpose.
    ...
    /// is a <see cref="GeContentFile"/>.
    /// </para>
    /// </remarks>
    [Tool]
    public partial class GeImportPlugin : EditorImportPlugin
    {
        /// <summary>The name Godot records in each <c>.import</c> file.</summary>
        public const string ImporterName = "gameplay_effects.ge";

        private const string ReportOption = "diagnostics/report_problems";

        public override string _GetImporterName() => ImporterName;

        public override string _GetVisibleName() => "Gameplay Effects Content";

        public override string[] _GetRecognizedExtensions() => new[] { "ge" };
            ...
            Godot.Collections.Array<string> genFiles)
        {
            ...
            if (opened != Error.Ok)
            {
                GD.PushError($"Gameplay Effects: could not read {sourceFile} ({opened}).");
                return opened;
            }

            if (ShouldReport(options)) Report(sourceFile, text);

            var resource = new GeContentFile
            {
