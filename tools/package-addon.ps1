...
$addon = Join-Path $root 'godot/GameplayEffects.Demo/addons/gameplay_effects'
...
try {
    $target = Join-Path $staging 'addons/gameplay_effects'
    ...
    $zip = Join-Path $OutputDirectory "gameplay-effects-godot-$version.zip"
    if (Test-Path $zip) { Remove-Item $zip }

    # Entries are written by hand rather than with Compress-Archive, which records Windows
    # separators. A zip full of "addons\gameplay_effects\..." unpacks as one long filename on Linux
